...
?>
      ...
      <section id="main-content">
          <section class="wrapper site-min-height">
              ...
              <?php
                 ...
                 $sql = "select e.u_id,o.*,e.e_id,d.e_id from order_prog e join orders o on o.o_id=e.o_id join editors d on d.e_id=e.e_id where e.u_id=$_GET[user] and o.o_id=$_GET[o_id] and d.e_id=$_SESSION[sess_id] and e.status=4 ";
                  ...
                  <div class="row mt">
                      <div class="col-lg-6">
                          <div class="content-panel">
							  <h4>Order Information <span class="label label-danger label-mini" >Revision</span></h4>
                      ...
                      <div class="col-lg-6">
                          <div class="content-panel">
                              ...
                              <?php 
                                ...
                                <?php 
                                require"../php/conn.php";
                                $sl= "select ed.attached,ed.o_id,o.o_id from order_prog ed join orders o on ed.o_id=o.o_id where ed.o_id=$_GET[o_id] and ed.status=4";
                                $ress= $conn->query($sl);
                                $row = $ress->fetch_assoc();
                                ...
                                <?php 
                                require"../php/conn.php";
                                $srl= "select ed.attach_rev,ed.o_id,o.o_id from order_prog ed join orders o on ed.o_id=o.o_id where ed.o_id=$_GET[o_id] and ed.status=4";
                                $ress= $conn->query($srl);
                                $row = $ress->fetch_assoc();
                                    ...
                                    echo ' <span class="label label-primary label-mini" >User: '.$row["u_name"].'</span>'; 
                                        ...
                                        <div id="collapseOne" style="padding:4px;" class="accordion-content collapse show text-left">
                                           ...
                                           <?php   require"..\php/conn.php";
                                            ...
                                            $sq="select m.o_id,m.u_id,m.message,m.date_created,m.e_id from messages m join orders o on m.o_id=o.o_id join users u on m.u_id=u.u_id join editors ed on m.e_id=ed.e_id where m.o_id=$_GET[o_id] and m.e_id=$_SESSION[sess_id] and m.u_id=$_GET[user]";
                                            $res= $conn->query($sq);
                                            // $row= $res->fetch_assoc();
 ...
 ?> 
                                        ...
                                        $sq="select m.o_id,m.u_id,m.u_message,m.date_created,m.e_id from messages m join orders o on m.o_id=o.o_id join users u on m.u_id=u.u_id join editors ed on m.e_id=ed.e_id where m.o_id=$_GET[o_id] and m.e_id=$_SESSION[sess_id] and m.u_id=$_GET[user]";
                                        $res= $conn->query($sq);
                                        // $row= $res->fetch_assoc();
                                        while($row= $res->fetch_assoc()){
                                        if($row['u_message'] !=''){
